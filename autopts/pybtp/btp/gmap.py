...
def gmap_ev_dummy_completed(gmap, data, data_len):
    logging.debug('%s %r', gmap_ev_dummy_completed.__name__, data)
    ...
    logging.debug(f'GMAP Dummy event completed: addr {addr} addr_type '
                  f'{addr_type} status {status}')

    gmap.event_received(defs.GMAP_EV_DUMMY_COMPLETED, (addr_type, addr, status))


GMAP_EV = {
    defs.GMAP_EV_DUMMY_COMPLETED: gmap_ev_dummy_completed,
}
